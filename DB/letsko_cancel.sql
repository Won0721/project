...
CREATE TABLE `cancel` (
  ...
  `ca_cancelDate` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '취소일자',
  `ca_cancelFee` int DEFAULT '0' COMMENT '취소수수료',
  `ca_refundAmount` int DEFAULT '0' COMMENT '반환금액',
  `ca_refundPoint` int DEFAULT '0' COMMENT '반환 마일리지',
  `ca_bk_num` int NOT NULL,
  PRIMARY KEY (`ca_num`),
  KEY `bk_numFK_idx` (`ca_bk_num`),
  CONSTRAINT `bk_numFK` FOREIGN KEY (`ca_bk_num`) REFERENCES `book` (`bk_num`) ON DELETE CASCADE ON UPDATE CASCADE
...
INSERT INTO `cancel` VALUES (18,'2023-11-23 12:43:22',6480,57320,1000,164);
